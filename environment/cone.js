...
function main() {
    ...
    object = generateCone(0, 0, 1, 0, 0, 0, 0.3, 0.3, 240, 91, 5);
    var cone_atas = new MyObject(object[0], object[1], shader_vertex_source, shader_fragment_source);

    object = generateTabung(0, -0.05, 0.1, 0, -0.05, 0, 0.38, 0, 0.38, 0.38, 0.07, 0.38, 125, 52, 10)
    ...
    var animate = function (time) {
        if (time > 0) {
            ...
            cone.setIdentityMove();      

            for (let i = 0; i < cone.child.length; i++) {
                cone.child[i].setIdentityMove();
            }

            glMatrix.mat4.rotateX(cone_atas.MOVEMATRIX, cone_atas.MOVEMATRIX, LIBS.degToRad(-90));
